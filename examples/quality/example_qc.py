...
def main():
    ...
    overall_quality = (
        "pass"
        if (
            qc_metrics["sequencing_quality"]["bases_above_q30"] > 0.8
            and qc_metrics["data_completeness"]["mapping_rate"] > 0.9
            and qc_metrics["biological_qc"]["contamination_estimate"] < 0.05
        )
        else "fail"
    )
    ...
    results = {
        ...
        "recommendations": [
            (
                "Data quality is acceptable for downstream analysis"
    ...
    io.dump_json({"quality_control": results}, results_file, indent=2)
