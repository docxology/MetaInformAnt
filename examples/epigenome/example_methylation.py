...
def main():
    ...
    results = {
        ...
        "site_classification": {
            ...
            "intermediate_sites": int(np.sum((np.mean(methylation_matrix, axis=1) >= 0.2) &
                                           (np.mean(methylation_matrix, axis=1) <= 0.8)))
        },
    ...
    print(f"Methylation range: {results['global_methylation']['range'][0]:.3f} - {results['global_methylation']['range'][1]:.3f}")
    print(f"Differentially methylated sites: {results['differential_methylation']['dm_sites_found']} ({results['differential_methylation']['dm_percentage']:.1f}%)")

    # Save results
    results_file = output_dir / "methylation_analysis.json"
    io.dump_json({
        "epigenome_analysis": results
