...
def test_learn_event_embeddings_basic():
    ...
    embeddings = learn_event_embeddings(sequences, embedding_dim=50, window_size=5, epochs=5, random_state=42)
...
def test_learn_event_embeddings_empty_sequences():
    ...
    with pytest.raises(ValueError, match="No events found in sequences"):
        learn_event_embeddings(sequences, embedding_dim=50, random_state=42)
...
def test_learn_event_embeddings_methods():
    ...
    # Skip-gram
    embeddings_sg = learn_event_embeddings(sequences, embedding_dim=50, method="skipgram", epochs=5, random_state=42)

    # CBOW
    embeddings_cbow = learn_event_embeddings(sequences, embedding_dim=50, method="cbow", epochs=5, random_state=42)
...
def test_sequence_embeddings_mean():
    ...
    np.testing.assert_array_almost_equal(seq_embeddings[0], np.array([0.25, 0.35, 0.45]))
    # Second sequence: just [0.7, 0.8, 0.9]
    np.testing.assert_array_almost_equal(seq_embeddings[1], np.array([0.7, 0.8, 0.9]))
...
def test_sequence_embeddings_sum():
    ...
    seq_embeddings = sequence_embeddings(sequences, event_embeddings, method="sum")

    assert seq_embeddings.shape == (1, 2)
    np.testing.assert_array_almost_equal(seq_embeddings[0], np.array([0.4, 0.6]))
...
def test_sequence_embeddings_max():
    ...
    seq_embeddings = sequence_embeddings(sequences, event_embeddings, method="max")

    assert seq_embeddings.shape == (1, 2)
    np.testing.assert_array_almost_equal(seq_embeddings[0], np.array([0.3, 0.4]))
...
def test_sequence_embeddings_temporal_weighting():
    ...
    seq_embeddings = sequence_embeddings(sequences, event_embeddings, method="mean", temporal_weighting=True)
...
def test_domain_specific_embeddings():
    ...
    domain_embeddings = domain_specific_embeddings(
        sequences, domains, embedding_dim=50, window_size=5, epochs=5, random_state=42
    )
...
def test_domain_specific_embeddings_empty_domain():
    ...
    domain_embeddings = domain_specific_embeddings(sequences, domains, embedding_dim=50, random_state=42)
...
def test_learn_event_embeddings_verbose(capsys):
    ...
    learn_event_embeddings(sequences, embedding_dim=50, epochs=3, verbose=True, random_state=42)
...
"""
    ...
    results = analyze_life_course(
        sequences, config_path=config_file, config_obj=config_obj, output_dir=tmp_path / "output"
    )
