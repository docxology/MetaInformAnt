...
class TestTissueConfiguration:
    ...
    def test_tissue_mapping_integrity(self):
        ...
        for canonical, synonyms in mapping.items():
            ...
            # Check consistency
            assert (
                canonical in synonyms
            ), f"Canonical name '{canonical}' should be in its own synonym list for completeness"
    ...
    def test_cross_file_consistency(self):
        ...
        if patches.get("bioprojects"):
            for bp, tissue in patches["bioprojects"].items():
                assert (
                    tissue in valid_tissues
                ), f"BioProject {bp} uses undefined tissue '{tissue}'. Add to tissue_mapping.yaml first."
        ...
        if patches.get("samples"):
            for sample, tissue in patches["samples"].items():
                assert (
                    tissue in valid_tissues
                ), f"Sample {sample} uses undefined tissue '{tissue}'. Add to tissue_mapping.yaml first."
