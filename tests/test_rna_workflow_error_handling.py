...
class TestManifestTracking:
    ...
    def test_manifest_records_steps_in_order(self, tmp_path: Path):
        ...
        if manifest_path.exists():
            ...
            manifest_steps = [r["step"] for r in records if r.get("step") in expected_step_names]
...
class TestWorkflowResume:
    ...
    def test_manifest_can_be_used_for_resume(self, tmp_path: Path):
        ...
        if manifest_path.exists():
            ...
            completed_steps = [r["step"] for r in records if r.get("return_code") == 0]
            failed_steps = [
                r["step"] for r in records if r.get("return_code") not in (0, 126, 204)  # 126=skip, 204=already done
            ]
...
class TestEarlyExitAndValidation:
    ...
    def test_pre_step_validation_integrate(self, tmp_path: Path):
        ...
        metadata_dir.mkdir(parents=True, exist_ok=True)
        metadata_file = metadata_dir / "metadata_selected.tsv"
        write_delimited([{"run": "SRR123456", "species": "Test_species"}], metadata_file, delimiter="\t")
