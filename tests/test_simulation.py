...
def test_simulate_counts_parameters():
    ...
    low_counts = simulate_counts_negative_binomial(
        num_genes=5, num_samples=5, mean_expression=10.0, rng=rng
    )
    rng = random.Random(42)  # Reset for fair comparison
    high_counts = simulate_counts_negative_binomial(
        num_genes=5, num_samples=5, mean_expression=1000.0, rng=rng
    )
