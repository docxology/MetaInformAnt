...
class TestIntegratedWorkflow:
    ...
    def test_workflow_no_args_is_helpful(self):
        ...
        assert (
            "required" in result.stderr.lower() or "config" in result.stderr.lower() or "usage" in result.stderr.lower()
        )
...
class TestDependencies:
    ...
    def test_wget_available(self):
        ...
        result = subprocess.run(["which", "wget"], capture_output=True, timeout=5)
    ...
    def test_kallisto_available(self):
        ...
        result = subprocess.run(["which", "kallisto"], capture_output=True, timeout=5)
        if result.returncode != 0:
    ...
    def test_wget_supports_continue(self):
        ...
        result = subprocess.run(["wget", "--help"], capture_output=True, text=True, timeout=5)
        if result.returncode == 0:
