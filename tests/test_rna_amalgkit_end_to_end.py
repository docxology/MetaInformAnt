...
class TestAmalgkitEndToEnd:
    ...
    def test_metadata_to_config_workflow(self, ensure_amalgkit_available):
        ...
        result = amalgkit.metadata(metadata_params, work_dir=str(work_dir), log_dir=str(work_dir / "logs"), check=False)
        ...
        result = amalgkit.config(config_params, work_dir=str(work_dir), log_dir=str(work_dir / "logs"), check=False)
    ...
    def test_workflow_step_sequence(self, ensure_amalgkit_available):
        ...
        for step_name, params in steps_to_test:
            ...
            result = step_func(params, work_dir=str(work_dir), log_dir=str(work_dir / "logs"), check=False)
    ...
    def test_complete_mini_workflow(self, ensure_amalgkit_available):
        ...
        result = amalgkit.config(
            {"out_dir": str(work_dir)}, work_dir=str(work_dir), log_dir=str(work_dir / "logs"), check=False
        )
    ...
    def test_all_steps_execute_in_order(self, ensure_amalgkit_available):
        ...
        cfg = AmalgkitWorkflowConfig(
            ...
            per_step={
                ...
                "cstmm": {"orthogroup_table": "dummy_orthologs.tsv"},
            },
        )
...
class TestAmalgkitUtilities:
    ...
    def test_build_amalgkit_command(self):
        ...
        command = amalgkit.build_amalgkit_command("metadata", {"out_dir": "/tmp", "threads": 2})
