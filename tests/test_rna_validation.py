...
class TestValidateAllSamples:
    ...
    def test_stage_specific_validation(self, tmp_path: Path):
        ...
        config = AmalgkitWorkflowConfig(
            work_dir=work_dir, extra_config={"steps": {"getfastq": {"out_dir": str(fastq_base)}}}
        )
    ...
    def test_all_stages_validation(self, tmp_path: Path):
        ...
        config = AmalgkitWorkflowConfig(
            work_dir=work_dir,
            extra_config={
                "steps": {"getfastq": {"out_dir": str(tmp_path / "fastq")}, "quant": {"out_dir": str(quant_dir)}}
            },
        )
...
class TestSaveValidationReport:
    ...
    def test_save_report(self, tmp_path: Path):
        ...
        validation_result = {"total_samples": 10, "validated": 8, "failed": 2, "per_sample": {}}
...
class TestEdgeCases:
    ...
    def test_missing_directories(self, tmp_path: Path):
        ...
        assert status["stage"] == "not_started"
        assert not any([status["download"], status["extraction"], status["quantification"], status["merge"]])
