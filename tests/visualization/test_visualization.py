...
class TestVisualization:
    ...
    def test_correlation_heatmap(self):
        ...
        np.random.seed(42)
        data = pd.DataFrame(
            {"A": np.random.randn(100), "B": np.random.randn(100), "C": np.random.randn(100), "D": np.random.randn(100)}
        )
    ...
    def test_volcano_plot(self):
        ...
        # Create test data
        data = pd.DataFrame({"log2FoldChange": np.random.randn(100), "padj": 10 ** (-np.random.exponential(2, 100))})
    ...
    def test_plot_data_types(self):
        ...
        data = pd.DataFrame({"A": [1.0, 2.0, 3.0], "B": [4, 5, 6], "C": ["a", "b", "c"]})  # Non-numeric column
    ...
    def test_manhattan_plot(self):
        ...
        data = pd.DataFrame(
            {"chromosome": chromosomes, "position": positions, "p_value": p_values, "log_p": -np.log10(p_values)}
        )
