...
def call_variants(bam_files: list[Path], reference: Path, output_vcf: Path, threads: int = 8):
    ...
    try:
        ...
        # Get variant count
        result = subprocess.run(f"bcftools view -H {output_vcf} | wc -l",
...
def main():
    ...
    parser.add_argument("--config", type=Path,
                       default=Path("config/gwas/gwas_amellifera.yaml"),
                       help="GWAS configuration file")
    parser.add_argument("--download-only", action="store_true",
                       help="Only download SRA data, don't run analysis")
    parser.add_argument("--skip-download", action="store_true",
                       help="Skip SRA download (use existing data)")
    parser.add_argument("--skip-align", action="store_true",
                       help="Skip alignment (use existing BAM files)")
    parser.add_argument("--skip-calling", action="store_true",
                       help="Skip variant calling (use existing VCF)")
    parser.add_argument("--comprehensive-plots", action="store_true", default=True,
                       help="Generate comprehensive visualization suite")
    parser.add_argument("--threads", type=int, default=8,
                       help="Number of threads")
    parser.add_argument("--sra-accessions", nargs="+",
