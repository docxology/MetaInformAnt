...
for trait_idx, (trait_name, test_type, trait_label) in enumerate(traits_to_test, 1):
    ...
    for idx in range(n_variants_to_test):
        ...
        if (idx + 1) % 1000 == 0:
            ...
            print(
                f"    ... {idx+1:,}/{n_variants_to_test:,} variants ({pct:.0f}%, {rate:.0f} var/s, ~{remaining:.0f}s remaining)"
            )

    trait_time = time.time() - trait_start
    print(
        f"    ✓ {len(assoc_results):,} successful tests ({trait_time:.1f}s, {len(assoc_results)/trait_time:.0f} var/s)"
    )
