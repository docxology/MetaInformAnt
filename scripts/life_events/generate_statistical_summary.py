...
def calculate_sequence_statistics(sequences: List[EventSequence]) -> Dict[str, Any]:
    ...
    return {
        ...
        "events_by_domain": {
            domain: len(events) for domain, events in events_by_domain.items()
        },
        "top_event_types": dict(
            Counter(events_by_type).most_common(20)
        ),
    }
...
def calculate_cooccurrence_statistics(sequences: List[EventSequence]) -> Dict[str, Any]:
    ...
    return {
        "top_event_cooccurrences": {
            f"{k[0]}|{k[1]}": v
            for k, v in sorted(cooccurrence.items(), key=lambda x: x[1], reverse=True)[:30]
        },
        "domain_cooccurrences": {
            f"{k[0]}|{k[1]}": v
            for k, v in domain_cooccurrence.items()
        },
    }
...
def main():
    ...
    logger.info(f"   Mean temporal span: {sequence_stats['temporal_spans_days']['mean']:.1f} days" if sequence_stats['temporal_spans_days']['mean'] else "   Mean temporal span: N/A")
