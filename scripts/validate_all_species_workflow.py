...
def test_workflow_planning(config_path: Path) -> Tuple[bool, str]:
    ...
    try:
        ...
        expected_steps = ["metadata", "config", "select", "getfastq", "integrate", 
...
def test_script_availability() -> Tuple[bool, str]:
    ...
    scripts = [
        REPO_ROOT / "scripts" / "rna" / "run_multi_species.py",
        REPO_ROOT / "scripts" / "rna" / "workflow_ena_integrated.py",
        REPO_ROOT / "scripts" / "rna" / "batch_download_species.py",
    ]
...
def main():
    ...
    if valid_configs:
        ...
        print(f"  ✅ Thread range: {min(thread_counts)} - {max(thread_counts)} (default: {thread_counts[0] if thread_counts else 'N/A'})")
    ...
    if valid_configs and venv_ok:
        ...
        print(f"  ✅ Command: export AK_THREADS=12 && python3 scripts/rna/run_multi_species.py")
        print(f"  ✅ Or individually: python3 scripts/rna/workflow_ena_integrated.py --config <config> --threads 12")
        results["passed"].append(("Startup Capability", f"{len(valid_configs)} species ready"))
    else:
        if not venv_ok:
            print(f"  ❌ Cannot start: Virtual environment not set up")
            results["failed"].append(("Startup Capability", "Virtual environment missing"))
        if not valid_configs:
            print(f"  ❌ Cannot start: No valid configs")
