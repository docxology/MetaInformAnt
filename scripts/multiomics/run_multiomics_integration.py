...
def run_workflow(args):
    ...
    try:
        ...
        logger.info(f"Integrated {omics_data.n_samples} samples across {len(omics_data.layer_names)} layers: {omics_data.layer_names}")
    ...
    if args.canonical_correlation:
        if len(omics_data.layer_names) >= 2:
            try:
                ...
                results = {
                    "layer_pair": [layer1, layer2],
                    "canonical_correlations": correlations[:10].tolist() if len(correlations) >= 10 else correlations.tolist(),
                ...
                logger.info(f"Canonical correlation analysis complete (max correlation: {results['max_correlation']:.3f})")
            except Exception as e:
