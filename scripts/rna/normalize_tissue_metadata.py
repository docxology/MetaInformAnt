...
def main():
    parser = argparse.ArgumentParser(description="Normalize tissue metadata values to canonical forms")
    parser.add_argument("--input", "-i", required=True, help="Path to input metadata TSV file")
    parser.add_argument(
        "--mapping",
        "-m",
    ...
    parser.add_argument(
        "--report-missing", "-s", action="store_true", help="Print report of samples with missing tissue"
    )
    parser.add_argument("--list-canonical", action="store_true", help="List canonical tissue names and exit")
    parser.add_argument("--dry-run", "-n", action="store_true", help="Don't write output file, just report statistics")
    ...
    df_normalized = apply_tissue_normalization(
        df, mapping_path=args.mapping, patches_path=args.patches if Path(args.patches).exists() else None
    )
