...
NUM_WORKERS = 10  # Reduced - use ENA pipeline for high parallelism instead
...
def download_sample(sample_id: str, sample_dir: Path) -> tuple[bool, str]:
    ...
    result = subprocess.run(cmd, capture_output=True, text=True, timeout=14400)  # 4 hour timeout for large files
...
def extract_fastq(sample_id: str, sample_dir: Path) -> tuple[bool, str]:
    ...
    for fq in sample_dir.glob("*.fastq"):
        subprocess.run(["gzip", "-f", str(fq)], capture_output=True, timeout=600)
        ...
        for row in reader:
            ...
            try:
                ...
                if size_bytes > MAX_SRA_BYTES:
                    skipped_large += 1
                    logger.warning(
                        f"Skipping {sample_id}: size {size_bytes/1024**3:.1f}GB > {MAX_SRA_BYTES/1024**3:.1f}GB limit"
                    )
                    continue
            ...
            for i, future in enumerate(as_completed(futures), 1):
                ...
                try:
                    ...
                    if result["success"]:
                        ...
                        logger.warning(f"[{i}/{len(to_process)}] ✗ {sample_id}: " f"{result['error']}")
                        # Flush log handlers for real-time visibility
                        for handler in logger.handlers:
                            handler.flush()
