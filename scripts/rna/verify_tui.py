...
from metainformant.core.ui.tui import TerminalInterface, GREEN, YELLOW, BLUE
...
def test_tui():
    ...
    try:
        for i in range(101):
            ...
            if i > 20:
                ui.update("task2", current=float(i-20), status="Downloading", speed=f"{(i-20)*0.8:.1f} MB/s", color=YELLOW)
            if i > 50:
                ui.update("task3", current=float(i-50), status="Downloading", speed=f"{(i-50)*1.2:.1f} MB/s", color=GREEN)
