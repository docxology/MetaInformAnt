...
def generate_html_report(results: Dict[str, Any], output_dir: Path) -> None:
    ...
    if failed:
        html_parts.append(
            f'                <button class="tab" onclick="showTab(\'failed\')">Failed ({len(failed)})</button>'
        )
    if errors:
        html_parts.append(
            f'                <button class="tab" onclick="showTab(\'errors\')">Errors ({len(errors)})</button>'
        )
    if passed:
        html_parts.append(
            f'                <button class="tab" onclick="showTab(\'passed\')">Passed ({len(passed)})</button>'
        )
...
def _generate_results_html(results: List[Dict[str, Any]]) -> str:
    ...
    for result in sorted(results, key=lambda x: (x["domain"], x["filename"])):
        ...
        if result.get("traceback"):
            error_info += (
                f'<div class="result-error"><strong>Traceback:</strong><br>{result["traceback"][:500]}...</div>'
            )
