...
def simulate_variants(
    ...
    vcf_lines = [
        ...
        f"#CHROM\tPOS\tID\tREF\tALT\tQUAL\tFILTER\tINFO\tFORMAT\t" + "\t".join([f"sample_{i:03d}" for i in range(n_samples)]),
    ]
    ...
    for pos_idx, pos in enumerate(positions):
        ...
        vcf_line = f"{chromosome}\t{pos}\tvariant_{pos_idx:05d}\t{ref}\t{alt}\t.\tPASS\t.\tGT:DP\t" + "\t".join(genotypes)
    ...
    try:
        if args.type == "variants":
            results = simulate_variants(
                output_dir, args.n_variants, args.n_samples, args.chromosome, args.seed
            )
            ...
            results = simulate_phenotype(
                output_dir, args.n_samples, args.n_causal, args.heritability, args.seed
            )
        elif args.type == "population":
            results = simulate_population(
                output_dir, args.n_samples, args.n_populations, args.fst, args.seed
            )
