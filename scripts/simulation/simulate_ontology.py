...
Examples:
  ...
  %(prog)s --type enrichment --n-genes 500 --n-terms 50 --n-significant 10
    ...
    parser.add_argument(
        "--annotations-per-gene", type=int, default=3, help="Average annotations per gene (annotations type)"
    )
    ...
    try:
        if args.type == "annotations":
            results = simulate_annotations(output_dir, args.n_genes, args.n_terms, args.annotations_per_gene, args.seed)
        elif args.type == "enrichment":
            results = simulate_enrichment(output_dir, args.n_genes, args.n_terms, args.n_significant, args.seed)
