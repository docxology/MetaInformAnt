  ...
  %(prog)s --type chipseq --n-peaks 500 --chromosome chr1 --length 1000000
    ...
    try:
        if args.type == "methylation":
            results = simulate_methylation(output_dir, args.n_sites, args.chromosome, args.mean_coverage, args.seed)
        elif args.type == "chromatin":
            results = simulate_chromatin(output_dir, args.chromosome, args.length, args.n_regions, args.seed)
        elif args.type == "chipseq":
            results = simulate_chipseq(output_dir, args.n_peaks, args.chromosome, args.length, args.seed)
