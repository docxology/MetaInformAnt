...
def _import_existing_functions() -> None:
    ...
    try:
        from metainformant.visualization.genomics.genomics import manhattan_plot as mp, volcano_plot as vp

        _original_volcano_plot = vp
        _original_manhattan_plot = mp
    except ImportError as e:
        logger.debug(f"Genomics plotting module not available: {e}")
