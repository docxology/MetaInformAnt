...
def plot_overrepresented_sequences(
    ...
    **kwargs
    ...
    if not overrepresented_seqs:
        ax.text(0.5, 0.5, 'No overrepresented sequences found',
...
def plot_multiomics_quality_overview(
    ...
    **kwargs
    ...
    for omics_type in omics_types:
        ...
        score = (report.get('overall_quality') or
                report.get('quality_score') or
                report.get('mean_quality') or
                np.mean(list(report.values())) if report else 0)
...
def plot_error_profiles(
    ...
    **kwargs
    ...
    for i, (error_type, profile) in enumerate(error_profiles.items()):
        ...
        ax.plot(positions, profile, color=colors[i], linewidth=2,
               label=error_type, alpha=0.8)
    ...
    if 'batch_pca' in batch_qc_data:
        ...
        if 'pc1' in pca_data and 'pc2' in pca_data and 'batches' in pca_data:
            ...
            for i, batch in enumerate(unique_batches):
                ...
                axes[plot_idx].scatter(np.array(pca_data['pc1'])[mask],
