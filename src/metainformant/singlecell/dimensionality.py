...
def compute_pca(
    ...
    print(f"PCA computed: {n_components} components, explaining {pca.explained_variance_ratio_.sum():.3f} of total variance")
...
def compute_neighbors(
    ...
    if method == 'sklearn':
        ...
        nbrs = NearestNeighbors(
            n_neighbors=n_neighbors + 1,  # +1 because each point is its own neighbor
            metric=metric,
            algorithm='auto'
        ).fit(X_use)
