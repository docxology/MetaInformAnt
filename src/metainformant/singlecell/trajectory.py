...
def lineage_analysis(
    ...
    results['transition_matrix'] = pd.DataFrame(
        transition_matrix,
    ...
    for end_cluster in end_clusters:
        ...
        while current != end_cluster and current not in visited:
            ...
            for next_cluster in unique_clusters:
                if (next_cluster not in visited and 
                    cluster_pseudotime.loc[next_cluster, 'mean'] > current_pseudotime):
                    transition_prob = transition_matrix[
                        cluster_to_idx[current], 
                        cluster_to_idx[next_cluster]
                    ]
