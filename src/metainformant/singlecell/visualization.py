        ...
        if color_var is None:
            ...
            if pd.api.types.is_categorical_dtype(color_data) or color_data.dtype == "object":
                ...
                for j, cat in enumerate(categories):
                    mask = color_data == cat
                    ax.scatter(x[mask], y[mask], s=size, alpha=alpha, c=[colors[j]], label=str(cat), edgecolors="none")

                if legend and len(categories) < 20:  # Only show legend for reasonable number of categories
                    ax.legend(bbox_to_anchor=(1.05, 1), loc="upper left", frameon=False)
            else:
                # Continuous coloring
                scatter = ax.scatter(x, y, s=size, alpha=alpha, c=color_data, cmap="viridis", edgecolors="none")
                cbar = plt.colorbar(scatter, ax=ax)
            ...
            scatter = ax.scatter(x, y, s=size, alpha=alpha, c=expression, cmap="Reds", edgecolors="none")
            cbar = plt.colorbar(scatter, ax=ax)
...
def plot_gene_expression(
    ...
    for i, gene in enumerate(genes):
        ...
        try:
            ...
            # Plot
            scatter = ax.scatter(x, y, s=size, alpha=alpha, c=expression, cmap=cmap, edgecolors="none")
            cbar = plt.colorbar(scatter, ax=ax)
...
def plot_clusters(
    ...
    for i, cluster in enumerate(unique_clusters):
        ...
        ax.scatter(x[mask], y[mask], s=size, alpha=alpha, c=[colors[i]], label=f"Cluster {cluster}", edgecolors="none")
