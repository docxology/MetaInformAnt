...
def maf_distribution(maf_values: List[float], output_file: Optional[str | Path] = None,
                    title: str = "Minor Allele Frequency Distribution") -> Optional[Any]:
    ...
    ax1.axvline(x=np.mean(maf_values), color='red', linestyle='--',
    ...
    maf_labels = ['Rare (<1%)', 'Low (1-5%)', 'Common (5-10%)',
                  'High (10-20%)', 'Very High (20-50%)']
    ...
    for bar, count in zip(bars, maf_counts):
        ax2.text(bar.get_x() + bar.get_width()/2, bar.get_height() + max(maf_counts) * 0.01,
...
def variant_density_plot(variant_positions: List[int], chromosome_lengths: Dict[str, int],
                        output_file: Optional[str | Path] = None,
                        title: str = "Variant Density Across Genome") -> Optional[Any]:
    ...
    for i, (chrom, (start, end)) in enumerate(zip(chroms, chrom_boundaries)):
        ...
        ax.text(chrom_centers[i], max(hist) * 0.9, chrom,
...
def hwe_deviation_plot(hwe_p_values: List[float], output_file: Optional[str | Path] = None,
                      title: str = "HWE Deviation Plot") -> Optional[Any]:
...
def missingness_plot(vcf_data: Dict[str, Any], output_file: Optional[str | Path] = None,
                    title: str = "Missingness Plot") -> Optional[Any]:
    ...
    # Add statistics
    ax1.axvline(x=np.mean(sample_missingness), color='red', linestyle='--',
               label=f'Mean: {np.mean(sample_missingness):.3f}')
    ax1.legend()
    ...
    # Add statistics
    ax2.axvline(x=np.mean(variant_missingness), color='red', linestyle='--',
               label=f'Mean: {np.mean(variant_missingness):.3f}')
    ax2.legend()
...
def transition_transversion_plot(ts_tv_data: Dict[str, Any], output_file: Optional[str | Path] = None,
                                title: str = "Transition/Transversion Plot") -> Optional[Any]:
    ...
    ax2.axhline(y=np.mean(ts_tv_ratio), color='red', linestyle='--',
               label=f'Mean: {np.mean(ts_tv_ratio):.2f}')
