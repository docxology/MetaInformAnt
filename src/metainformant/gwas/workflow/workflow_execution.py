...
def _sample_major_to_variant_major(
    genotypes_by_sample: List[List[Any]],
...
def _lambda_gc_from_associations(
    assoc_results: List[Dict[str, Any]],
) -> Optional[float]:
    ...
    if lambda_gc is None:
        return None
    rounded = round(lambda_gc, 4)
    return rounded if rounded > 0.0 else None


def _apply_qvalue_and_bonferroni_annotations(
    assoc_results: List[Dict[str, Any]],
) -> None:
...
def _apply_full_correction_outputs(
    association_results: List[Dict[str, Any]],
    correction_method: Optional[str] = None,
) -> Dict[str, Any]:
    """Run the configured correction stack and annotate association rows.

    ``correction_method`` selects which correction produces outputs and row
    annotations: "bonferroni", "fdr", or "genomic_control". ``None`` (no
    ``[correction][method]`` configured) runs the full stack for backward
    compatibility. Unrecognized values fall back to the full stack with a
    warning; config validation rejects invalid methods earlier.
    """
