...
def execute_gwas_workflow(
    ...
    try:
        ...
        if p_values:
            ...
            n_bonf = (
                bonf_result.get("n_significant", 0)
            ...
            n_fdr = (
                fdr_result.get("n_significant", 0)
            ...
            results["outputs"]["bonferroni"] = (
                bonf_result if isinstance(bonf_result, dict) else {}
            )
            results["outputs"]["fdr"] = (
                fdr_result if isinstance(fdr_result, dict) else {}
            )
            results["outputs"]["genomic_control"] = {"lambda_gc": lambda_gc}

            # Mark significant variants in association results
            bonf_significant = (
                bonf_result.get("significant", [])
                if isinstance(bonf_result, dict)
                else []
            )
            fdr_significant = (
                fdr_result.get("significant", [])
                if isinstance(fdr_result, dict)
                else []
            )
            fdr_adjusted = (
...
def run_gwas(
    ...
    try:
        ...
        if sample_list or sample_subset:
            ...
            if sample_subset:
                metadata_path_sub = config.get("metadata_file") or (
                    config.get("samples", {}).get("metadata_file")
        ...
        if isinstance(haplodiploidy_config, dict) and haplodiploidy_config.get(
            "enabled", False
        ):
            logger.info("Step 2b: Checking haplodiploidy")
            het_threshold = haplodiploidy_config.get("het_threshold", 0.05)
            haplo_result = check_haplodiploidy(
                filtered_data, het_threshold=het_threshold
            )
                ...
                if genotypes_by_sample and diploid_indices:
                    ...
                    if old_samples:
                        filtered_data["samples"] = [
                            old_samples[i] for i in diploid_indices
                        ]
        ...
        if genotypes_by_sample and genotypes_by_sample[0]:
            ...
            genotypes_by_variant = [
                [genotypes_by_sample[s][v] for s in range(n_samples)]
        ...
        metadata_path = (
            ...
            or config.get("samples", {}).get("metadata_file")
            if isinstance(config.get("samples"), dict)
            else None
        )
        ...
        if genotypes_by_variant and phenotypes:
            if model == "mixed":
                ...
                if kinship_result and kinship_result.get("status") == "success":
                    ...
                    logger.warning(
                        "Kinship matrix unavailable, falling back to linear model"
                    )
                    model = "linear"
                    ...
                    try:
                        ...
                        if model == "logistic":
                            ...
                            result = association_test_linear(
                                geno_trimmed, pheno_trimmed
                            )

                        result["variant_id"] = f"variant_{i}"
                        if filtered_data.get("variants") and i < len(
                            filtered_data["variants"]
                        ):
            ...
            if valid_p:
                ...
                if median_p > 0:
                    ...
                    if lambda_gc > 1.1:
                        ...
                        logger.info(
                            f"Lambda GC = {lambda_gc:.3f} (within expected range)"
                        )
        ...
        if assoc_results and filtered_data.get("variants"):
            ...
            try:
                ...
                write_significant_hits(
                    assoc_results, variant_info, sig_path, threshold=threshold
                )

                # Write JSON summary
                summary_path = output_dir / "results_summary.json"
                summary = create_results_summary(
                    assoc_results, summary_path, threshold=threshold
                )
        ...
        try:
            from metainformant.gwas.visualization.interactive.suite import (
                generate_all_plots as gen_plots,
            )
            ...
            try:
                ...
                if genotypes_by_variant and phenotypes:
                    if (
                        model == "mixed"
                        and kinship_result
                        and kinship_result.get("status") == "success"
                    ):
                        assoc_results = run_mixed_model_gwas(
                            genotype_matrix=genotypes_by_variant,
                            phenotypes=phenotypes[:min_s],
                        ...
                        for i, genotype in enumerate(genotypes_by_variant):
                            try:
                                result = association_test_linear(
                                    genotype[:min_s], phenotypes[:min_s]
                                )
                                result["variant_id"] = f"variant_{i}"
                                if filtered_data.get("variants") and i < len(
                                    filtered_data["variants"]
