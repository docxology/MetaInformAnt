...
The CLI wrapper lives in ``scripts/gwas/run_amellifera_gwas.py``.
"""
...
def main(argv: list[str] | None = None) -> tuple[dict, dict]:
    ...
    if ld_result:
        print(
            f"  LD pruning:          {ld_result.get('variants_before', 'N/A')} -> "
            f"{ld_result.get('variants_after', 'N/A')}"
        )
