...
Runs the full GWAS pipeline from data validation through association testing,
...
The CLI wrapper lives in ``scripts/gwas/pipelines/run_pbarbatus_gwas.py``.
"""
...
from metainformant.gwas.visualization._general_impl import manhattan_plot, qq_plot
from metainformant.gwas.visualization.structure_plots import (
...
def run(config_path: str = "config/gwas/gwas_pbarbatus_synthetic.yaml") -> None:
    """Run the complete end-to-end P. barbatus GWAS analysis."""
    ...
    print(
        f'  Traits: {", ".join([c for c in pheno_df.columns if c not in ["sample_id", "age_days", "colony", "sex"]])}'
    )
    ...
    print(
        f"OPTIMIZATION: Testing {n_variants_to_test:,} variants × {len(traits_to_test)} traits = {total_tests:,} tests"
    )
    ...
    for trait_idx, (trait_name, test_type, trait_label) in enumerate(traits_to_test, 1):
        ...
        for idx in range(n_variants_to_test):
            ...
            if (idx + 1) % 1000 == 0:
                ...
                print(
                    f"    ... {idx+1:,}/{n_variants_to_test:,} variants ({pct:.0f}%, "
                    f"{rate:.0f} var/s, ~{remaining:.0f}s remaining)"
                )

        trait_time = time.time() - trait_start
        print(
            f"    ✓ {len(assoc_results):,} successful tests "
            f"({trait_time:.1f}s, {len(assoc_results)/trait_time:.0f} var/s)"
        )
    ...
    for trait_name, trait_data in all_results.items():
        ...
        try:
            manhattan_result = manhattan_plot(
                ...
                str(manhattan_file),
                title=(
                    f"GWAS Manhattan Plot: {trait_label}\nP. barbatus "
                    f"(n={n_samples} samples, {len(assoc_results):,} variants)"
                ),
            )
        ...
        try:
            qq_result = qq_plot(
                [r["p_value"] for r in assoc_results], str(qq_file), title=f"Q-Q Plot: {trait_label}\nP. barbatus GWAS"
            )
