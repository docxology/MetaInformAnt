...
class WorkflowManager:
    ...
    def add_sample(self, sample_id: str, sra_url: str, dest_path: Path):
        ...
        self.ui.add_bar(
            sample_id, 
            sample_id[:20],  # Truncate long IDs
            total=0.0, 
            unit="MB"
        )
    ...
    def _run_download_phase(self):
        ...
        for sample_id, state in self.samples.items():
            ...
            future = self.executor.submit(
                ...
                sample_id, state.sra_url, state.dest_path
            )
    ...
    def _download_sample(self, sample_id: str, url: str, dest: Path) -> bool:
        ...
        if dest.exists():
            ...
            if total_bytes > 0 and existing_size >= (total_bytes * 0.99):
                self.ui.update(sample_id, current=existing_mb, total=existing_mb, 
                             status="Cached ✓", color=GREEN)
                return True
            elif total_bytes == 0:
    ...
    def _run_getfastq_phase(self):
        ...
        for state in downloaded:
            ...
            self.ui.update(
                state.sample_id, 
                stage="Extract", 
                status="Extracting FASTQ...", 
                color=MAGENTA,
                current=0.0, total=100.0
            )
