...
from ..dna.fastq import FastqRecord, iter_fastq
...
def per_sequence_quality(reads: List[FastqRecord]) -> Dict[str, Any]:
    ...
    return {
        ...
        "quality_distribution": dict(zip([f"{edge:.1f}" for edge in bin_edges[:-1]], hist.tolist())),
    }
...
def gc_content_distribution(reads: List[FastqRecord]) -> Dict[str, Any]:
    ...
    return {
        ...
        "gc_distribution": dict(zip([f"{edge:.0f}" for edge in bin_edges[:-1]], hist.tolist())),
    }
