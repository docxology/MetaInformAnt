    ...
    **kwargs
    ...
    if HAS_SEABORN:
        ...
        sns.heatmap(corr_matrix, mask=mask, annot=True, fmt='.2f',
                   cmap='coolwarm', center=0, ax=ax, **kwargs)
    else:
...
def plot_life_course_trajectory(
    ...
    **kwargs
    ...
    for i, (age, event_type, desc) in enumerate(zip(ages, event_types, descriptions)):
        ...
        if desc:
            label += f": {desc}"
        ax.annotate(label, (age, 0), xytext=(0, 10 + i % 3 * 10),
    ...
    # Add legend
    legend_elements = [plt.scatter([], [], c=color, label=event, s=50, edgecolors='black')
...
def plot_behavioral_patterns(
    ...
    **kwargs
    ...
    for i, behavior in enumerate(behaviors):
        ...
        ax.hist(times, bins=30, alpha=0.7, color=colors[i],
               label=str(behavior), density=True)
...
def plot_phenotype_pca(
    ...
    **kwargs
    ...
    scatter = ax.scatter(pca_coords[:, 0], pca_coords[:, 1],
                        alpha=0.7, c=range(len(pca_coords)),
                        cmap='viridis', **kwargs)
    ...
    **kwargs
    ...
    if confidence_intervals:
        for i, trait in enumerate(traits):
            if trait in confidence_intervals:
                ...
                ax.errorbar(i, h2_values[i], yerr=[[h2_values[i]-lower], [upper-h2_values[i]]],
    ...
    for bar, h2 in zip(bars, h2_values):
        ax.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 0.01,
               f'{h2:.2f}', ha='center', va='bottom', fontsize=8)
    ...
    **kwargs
    ...
    for i, (species, events) in enumerate(species_data.items()):
        ...
        if ages:
            ...
            ax.plot(sorted_ages, cumulative, 'o-', color=colors[i],
                   label=species, linewidth=2, markersize=4, alpha=0.8)
...
def plot_phenotype_network(
    ...
    **kwargs
    ...
    nx.draw_networkx_labels(G, pos, {i: phenotype_names[i] for i in G.nodes()},
                           font_size=8, ax=ax)
...
def create_interactive_phenotype_browser(
    ...
    **kwargs
    ...
    fig = px.parallel_coordinates(
        phenotype_data,
        title="Interactive Phenotype Browser",
        **kwargs
    )
