    ...
    **kwargs
    ...
    for i in range(len(population_history[0])):
        ...
        ax.plot(time_points, trajectory, color=colors[i],
               linewidth=2, alpha=0.8, label=f'Population {i+1}')
    ...
    if 'agent_states' in agent_data:
        ...
        if isinstance(states_history, list):
            ...
            for state, counts in state_counts.items():
                axes[1].plot(time_points[:len(counts)], counts,
                           label=f'State {state}', linewidth=2, alpha=0.8)
    ...
    **kwargs
    ...
    if 'fitness_history' in evolution_data:
        ...
        if 'generations' in fitness_data and 'mean_fitness' in fitness_data:
            axes[plot_idx].plot(fitness_data['generations'], fitness_data['mean_fitness'],
                              'b-', linewidth=2, label='Mean Fitness', alpha=0.8)
            if 'max_fitness' in fitness_data:
                axes[plot_idx].plot(fitness_data['generations'], fitness_data['max_fitness'],
                                  'r--', linewidth=1, label='Max Fitness', alpha=0.7)
    ...
    if 'diversity_history' in evolution_data:
        ...
        if 'generations' in diversity_data and 'diversity' in diversity_data:
            axes[plot_idx].plot(diversity_data['generations'], diversity_data['diversity'],
    ...
    if 'population_size_history' in evolution_data:
        ...
        if 'generations' in pop_data and 'population_size' in pop_data:
            axes[plot_idx].plot(pop_data['generations'], pop_data['population_size'],
    ...
    if 'mutation_history' in evolution_data:
        ...
        if 'generations' in mut_data and 'mutation_count' in mut_data:
            axes[plot_idx].plot(mut_data['generations'], mut_data['mutation_count'],
...
def animate_population_dynamics(
    ...
    **kwargs
    ...
    anim = animation.FuncAnimation(fig, animate, frames=len(population_history),
...
def plot_simulation_validation_comparison(
    ...
    **kwargs
    ...
    ax.fill_between(range(len(sim_mean)), sim_mean - sim_std, sim_mean + sim_std,
                   alpha=0.3, color='red', label='Simulated ±1σ')
    ...
    **kwargs
    ...
    if 'time_series' in simulation_results:
        ...
        if 'time' in ts_data and 'values' in ts_data:
            fig.add_trace(
                go.Scatter(x=ts_data['time'], y=ts_data['values'], mode='lines+markers'),
                row=1, col=1
            )
    ...
    if 'distributions' in simulation_results:
        ...
        fig.add_trace(
            go.Histogram(x=dist_data, nbinsx=30),
            row=1, col=2
        )
    ...
    if 'correlations' in simulation_results:
        ...
        fig.add_trace(
            go.Heatmap(z=corr_data, colorscale='RdBu', zmid=0),
            row=2, col=1
        )
    ...
    fig.update_layout(
        title="Interactive Simulation Results Dashboard",
        **kwargs
    )
