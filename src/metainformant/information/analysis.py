...
from metainformant.information import syntactic, semantic, continuous, estimation
...
def information_profile(
    ...
    for i in range(seq_length - k + 1):
        ...
        entropy_val = estimation.entropy_estimator(
            kmer_counts, method=method, bias_correction=True
        )
    ...
    **kwargs: Any
    ...
    if method == "entropy":
        ...
        for i in range(n_features):
            entropy_val = continuous.differential_entropy(
                data[:, i], **kwargs
            )
        ...
        for i in range(n_features):
            for j in range(i + 1, n_features):
                mi_val = continuous.mutual_information_continuous(
    ...
    logger.info(
        f"Calculated information signature using {method}: "
        ...
        for i in range(seq_len - length + 1):
            ...
            if count > 1:
                ...
                already_recorded = any(
                    r['pattern'] == pattern and r['start'] == i
                    for r in repeats
                )
...
def _find_low_complexity_regions(
    sequence: str,
    window_size: int = 20,
    threshold: float = 0.7
) -> List[Dict[str, Any]]:
...
def compare_sequences_information(
    ...
    if method == "mutual_info":
        ...
        comparison['entropy_difference'] = [
            p1['entropy'] - p2['entropy']
            for p1, p2 in zip(profile1['profile'], profile2['profile'])
        ]
        ...
        for i in range(len(seq1) - k + 1):
            ...
            kld_val = estimation.kl_divergence_estimator(
                list(counts1.elements()), list(counts2.elements())
            )
