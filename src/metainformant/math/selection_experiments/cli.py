...
from .model import simulate_generation, simulate_generations, noise
from .plotting import display_s_vs_q, display_sq_vs_w, display_generation
...
def run_replay(dest_dir: Path) -> None:
    ...
    r1 = simulate_generation(np.random.normal(0, 1, 1_000_000), s_hat=.8)
    ...
    r2 = simulate_generation(np.random.normal(0, 1, 1_000_000), s_hat=.65)
...
def add_math_selection_subparser(subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    ...
    replay.add_argument("--dest", default="src/metainformant/math/selection_experiments",
    ...
    abstract.add_argument("--dest", default="src/metainformant/math/selection_experiments",
                        help="Destination directory for the abstract")
