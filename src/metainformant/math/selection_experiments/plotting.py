...
def display_s_vs_q(r: GenerationResult, *, sample_size: int = 600, dest: str | Path | None = None) -> Path | None:
    ...
    plt.plot(
        ...
        linewidth=3,
        linestyle="-",
        label=r"$\bar{\phi}(s)$ (true relationship)",
    )
    ...
    plt.title(
        f"Structural Trait vs Quality Signal\nSignal Fidelity: $\\hat{{s}}$ = {r.s_hat}",
        fontsize=20,
        fontweight="bold",
        pad=30,
        loc="center",
    ...
    legend = plt.legend(fontsize=16, loc="upper left", framealpha=0.9, bbox_to_anchor=(0.02, 0.98))
    ...
    if dest is not None:
        ...
        plt.savefig(out_path, format="png", dpi=200, bbox_inches="tight", facecolor="white", edgecolor="none")
    plt.close(fig)
    return out_path
...
def display_sq_vs_w(r: GenerationResult, *, sample_size: int = 600, dest: str | Path | None = None) -> Path | None:
    ...
    plt.plot(x_vals, fit_line_s(x_vals), color=colors["green"], linewidth=3, linestyle="-", alpha=0.8, label=".2f")
    plt.plot(x_vals, fit_line_q(x_vals), color=colors["yellow"], linewidth=3, linestyle="--", alpha=0.8, label=".2f")
    ...
    plt.title(
        f"Selection on Correlated Traits\nSignal Fidelity: $\\hat{{s}}$ = {r.s_hat}",
        fontsize=20,
        fontweight="bold",
        pad=30,
    )
    ...
    # Enhanced legend
    legend = plt.legend(fontsize=16, loc="upper right", framealpha=0.9, bbox_to_anchor=(0.98, 0.98))
    legend.get_frame().set_linewidth(2)

    # Add selection differential information
    selection_info = (
        f"Selection on Structure: {r.rho_ws:.3f}\n"
...
def display_gr(gr: GenerationsResult, *, dest: str | Path | None = None) -> Path | None:
    ...
    plt.fill_between(
        generations, s_means_norm - s_stds_norm, s_means_norm + s_stds_norm, color=colors["blue"], alpha=0.2
    )
    ...
    plt.fill_between(
        generations, q_means_norm - q_stds_norm, q_means_norm + q_stds_norm, color=colors["red"], alpha=0.2
    )

    plt.plot(
        generations, w_means_norm, color=colors["yellow"], linewidth=3, marker="^", markersize=6, label=r"$w$ (fitness)"
    )
    plt.fill_between(
        generations, w_means_norm - w_stds_norm, w_means_norm + w_stds_norm, color=colors["yellow"], alpha=0.2
    )
    ...
    plt.title(
        f"Trait Evolution Over {gr.generations} Generations\n"
    ...
    # Enhanced legend
    legend = plt.legend(fontsize=16, loc="center right", framealpha=0.9, bbox_to_anchor=(1.02, 0.5))
    legend.get_frame().set_linewidth(2)
...
def create_composite_abstract(experiment_results: dict, *, dest: str | Path | None = None) -> Path | None:
    ...
    # Create grid layout
    gs = fig.add_gridspec(3, 4, hspace=0.3, wspace=0.3, top=0.9, bottom=0.1, left=0.1, right=0.95)
    ...
    if "signal_mapping" in experiment_results:
        ...
        ax1.scatter(s_sample, q_sample, alpha=0.6, s=30, color=colors["green"], edgecolor="darkgreen", linewidth=0.5)
        s_line = np.linspace(np.min(s_sample), np.max(s_sample), 100)
    ...
    if "selection_correlation" in experiment_results:
        ...
        coeffs_s = np.polyfit(s_sample, w_sample, 1)
        coeffs_q = np.polyfit(s_from_q, w_sample, 1)
        x_vals = np.linspace(min(np.min(s_sample), np.min(s_from_q)), max(np.max(s_sample), np.max(s_from_q)), 100)
    ...
    if "rebound_evolution" in experiment_results:
        ...
        ax3.plot(
            generations, s_norm, color=colors["blue"], linewidth=3, marker="o", markersize=4, label="Structural (s)"
        )
        ax3.plot(generations, q_norm, color=colors["red"], linewidth=3, marker="s", markersize=4, label="Quality (q)")
        ax3.plot(
            generations, w_norm, color=colors["yellow"], linewidth=3, marker="^", markersize=4, label="Fitness (w)"
        )
    ...
    if "inverse_evolution" in experiment_results:
        ...
        ax4.plot(generations, q_norm, color=colors["red"], linewidth=3, marker="s", markersize=4, label="Quality (q)")
        ax4.plot(
            generations, w_norm, color=colors["yellow"], linewidth=3, marker="^", markersize=4, label="Fitness (w)"
        )
...
• Fitness landscapes shape signal evolution"""
    ...
    if dest is not None:
        out_path = Path(dest)
        core_io.ensure_directory(out_path.parent)
        plt.savefig(out_path, format="png", dpi=300, bbox_inches="tight", facecolor="white", edgecolor="none")
    plt.close(fig)
    return out_path
