...
def download_rna_fasta_from_ftp(accession: str, output_dir: Path, **kwargs: Any) -> Optional[Path]:
...
def download_cds_fasta_from_ftp(accession: str, output_dir: Path, **kwargs: Any) -> Optional[Path]:
...
def extract_transcripts_from_gff(
    ...
    cmd = [gffread, str(gff_file), "-g", str(genome_fasta), "-w", str(output_file)]
...
def prepare_transcriptome_for_kallisto(transcriptome_fasta: Path, output_fasta: Path, **kwargs: Any) -> Optional[Path]:
...
def build_kallisto_index(transcriptome_fasta: Path, index_path: Path, **kwargs: Any) -> Optional[Path]:
    ...
    cmd = [kallisto_cmd, "index", "-i", str(index_path), "-k", str(kmer_size), str(transcriptome_fasta)]
...
def orchestrate_genome_setup(
    config: Dict[str, Any], species_name: str, work_dir: Path, **kwargs: Any
) -> Dict[str, Any]:
