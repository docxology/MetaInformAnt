...
_NCBI_SETTINGS_LOCK = threading.Lock()
...
def _ensure_ncbi_settings_for_data_root(data_root: Path) -> Path:
    """Create a campaign-local SRA Toolkit settings file.

    ``fasterq-dump`` resolves an accession through the user's global VDB
    configuration when no ``NCBI_SETTINGS`` override is supplied.  A stale
    global setting can silently place multi-gigabyte SRA caches on the host
    system volume (for example, inside an old pytest temporary directory),
    even though the campaign data root is on the mounted research volume.
    Keep the fallback cache beside the campaign data and make the setting
    explicit for every subprocess.  The file is deterministic and contains
    only the setting required by this runner; user-global NCBI configuration
    ...
    payload = (
        "## Generated by MetaInformAnt streaming orchestration.\n"
        "## Keep large SRA fallback caches with the campaign data root.\n"
        f'/repository/user/main/public/root = "{sra_root}"\n'
    )

    try:
        # Multiple sample workers can enter fallback extraction together. The
        # settings path is shared by the campaign, so serialize its read/write
        # check and use a thread-unique temporary name; otherwise one worker
        # can replace another worker's temporary file between write and rename.
        with _NCBI_SETTINGS_LOCK:
            ...
            if not settings_path.is_file() or settings_path.read_text(encoding="utf-8") != payload:
                temporary = settings_path.with_name(
                    f".{settings_path.name}.tmp-{os.getpid()}-{threading.get_ident()}"
                )
                temporary.write_text(payload, encoding="utf-8")
                os.replace(temporary, settings_path)
    except OSError as exc:
        logger.warning(
            "Unable to prepare campaign-local NCBI settings under %s: %s; "
            "fasterq-dump will inherit the caller environment",
            root,
            exc,
        )
...
def _ncbi_environment_for_data_root(data_root: Path) -> dict[str, str]:
    """Return a subprocess environment with an explicit campaign SRA cache."""

    environment = os.environ.copy()
    environment["NCBI_SETTINGS"] = str(_ensure_ncbi_settings_for_data_root(data_root))
    return environment
...
def _raw_validation_marker_matches(
    ...
    if (
        recorded_effective_layout is not None
        and recorded_effective_layout != effective_layout
    ):
        return False
    ...
    if (
        recorded_reconciliation is not None
        and recorded_reconciliation != expected_reconciliation
    ):
        return False
    ...
    if (
        expected_reconciliation
        == "amalgkit_0.16.32_paired_metadata_single_fastq"
        and payload.get("layout_validation")
        != "full_fastq_record_scan_read1_only"
    ):
        return False
...
def _validated_local_fastq_inputs(
    ...
    *,
    ...
    unnumbered FASTQ is retained for Amalgkit 0.16.32's explicit
    ...
    if (
        expected_paired is True
        and len(candidates) == 1
        and candidates[0].name == f"{accession}.fastq.gz"
    ):
        ...
        # Amalgkit 0.16.32's read-1-only case is accepted only after a complete
        ...
        layout_validation = (
            "full_fastq_record_scan_interleaved_split"
            if len(candidates) == 2
            else "full_fastq_record_scan_read1_only"
        )
...
def _promote_staged_fastq_file(staged: Path, destination: Path) -> None:
    ...
    temporary = destination.with_name(
        f".{destination.name}.{os.getpid()}.{threading.get_ident()}.copying"
    )
    try:
        temporary.unlink(missing_ok=True)
    ...
    with gzip.open(source, "rt", encoding="utf-8", newline="") as reader:
        while True:
            ...
            if _fastq_header_explicit_mate(record[0]) == 2:
                raise ValueError(
                    f"non-adjacent mate-2 record in one-file paired input {source.name}"
                )
            record_count += 1
    if record_count == 0:
...
def _split_interleaved_fastq(
    ...
    if output_1.exists() or output_2.exists():
        raise FileExistsError(
            f"cannot split interleaved FASTQ over existing mate output(s) for {accession}"
        )
    ...
    try:
        with gzip.open(source, "rt", encoding="utf-8", newline="") as reader:
            with gzip.open(temporary_1, "wt", compresslevel=1, encoding="utf-8", newline="") as writer_1:
                with gzip.open(
                    temporary_2, "wt", compresslevel=1, encoding="utf-8", newline=""
                ) as writer_2:
                    while True:
                        ...
                        for record in (first, second):
                            ...
                            if not sequence or len(sequence) != len(quality):
                                raise ValueError(
                                    f"sequence/quality length mismatch in {source.name}"
                                )
                        ...
                        if first_mate == 2 or second_mate == 1:
                            raise ValueError(
                                f"explicit FASTQ mate order is reversed in {source.name}"
                            )
                        if (
                            first_mate is not None
                            and second_mate is not None
                            and (first_mate, second_mate) != (1, 2)
                        ):
                            raise ValueError(
                                f"explicit FASTQ mate labels are inconsistent in {source.name}"
                            )
...
def _metadata_requires_fastq_input_stats(metadata_path: Path, accession: str) -> bool:
    ...
    Amalgkit 0.16.32 infers ``spot_length`` from ``total_bases / total_spots``
...
def _write_fastq_input_stats(sample_dir: Path, accession: str) -> bool:
    ...
    try:
        fastq_files = sorted(
            ...
            if path.is_file()
            and path.name.startswith(accession)
            and path.name.endswith(_FASTQ_INPUT_SUFFIXES)
        )
    except OSError:
        return False
    if not fastq_files:
...
def _raw_input_priority_index(fastq_dir: Path, accessions: set[str]) -> dict[str, int]:
    ...
    return {
        accession: state[0]
        for accession, state in _raw_input_state_index(fastq_dir, accessions).items()
    }
...
def _prioritize_tasks_by_raw_state(
    ...
    *,
    ...
    states_by_dir = {
        fastq_dir: _raw_input_state_index(fastq_dir, accessions)
        for fastq_dir, accessions in accessions_by_dir.items()
    }
    for task in tasks:
    ...
    reusable_fastq = [
        task
        for task in ordered
        if task["_raw_input_priority"] == 0 and not task["_uses_local_sra"]
    ]
...
def _filter_valid_run_rows(df: Any, species_name: str, metadata_path: Path) -> Any:
    ...
    logger.warning(
        "%s: excluding %d metadata rows without a concrete %s accession; "
        "see %s",
        species_name,
        len(invalid),
        run_column,
...
def _filter_kallisto_ineligible_reads(
    dataframe: Any, kmer_size: int
) -> tuple[Any, Any, Any]:
            ...
            aliases.append(
                {
                    ...
                    "note": "Metadata target lacks an independent configured index; reads use the declared reference source.",
                }
            )
...
def _cache_index_directory(index_dir: str, species_name: str) -> str:
    ...
    try:
        source_files = sorted(
            path for path in source.glob("*.idx") if path.is_file() and path.stat().st_size > 0
        )
    except OSError as exc:
    ...
    with _INDEX_CACHE_LOCK:
        ...
        if (
            ...
            and all((target / name).is_file() and (target / name).stat().st_size == details["size"]
...
class StreamingPipelineOrchestrator:
    ...
    def __init__(self, config_dir: Path = CONFIG_DIR, log_dir: Path = LOG_DIR, db_path: Optional[Path] = None):
        ...
        self.system_root = Path(
            os.environ.get("AMALGKIT_SYSTEM_ROOT", "/System/Volumes/Data")
        ).expanduser()
        try:
        ...
        self.download_timeout_seconds = _duration_setting(
            "AMALGKIT_PIPELINE_DOWNLOAD_TIMEOUT_SECONDS", 7200
        )
        self.download_speed_limit_bytes = _positive_int_setting(
            "AMALGKIT_PIPELINE_DOWNLOAD_SPEED_LIMIT_BYTES", 1024
        )
        self.download_speed_time_seconds = _duration_setting(
            "AMALGKIT_PIPELINE_DOWNLOAD_SPEED_TIME_SECONDS", 600
        )
        self.download_retries = _nonnegative_int_setting(
            "AMALGKIT_PIPELINE_ENA_RETRIES", 5
        )
        self.download_retry_delay_seconds = _duration_setting(
            "AMALGKIT_PIPELINE_ENA_RETRY_DELAY_SECONDS", 5
        )
        self.ena_api_retries = _nonnegative_int_setting(
            "AMALGKIT_PIPELINE_ENA_API_RETRIES", 2
        )
        self.ena_api_retry_delay_seconds = _duration_setting(
            "AMALGKIT_PIPELINE_ENA_API_RETRY_DELAY_SECONDS", 2
        )
        self.fasterq_timeout_seconds = _duration_setting(
            "AMALGKIT_PIPELINE_FASTQ_TIMEOUT_SECONDS", 7200
        )
        self.sra_validation_timeout_seconds = _duration_setting(
            "AMALGKIT_PIPELINE_SRA_VALIDATE_TIMEOUT_SECONDS", 600
        )
        self.compression_timeout_seconds = _duration_setting(
            "AMALGKIT_PIPELINE_COMPRESSION_TIMEOUT_SECONDS", 1800
        )
        self.quant_timeout_seconds = _duration_setting(
            "AMALGKIT_PIPELINE_QUANT_TIMEOUT_SECONDS", 7200
        )
        ...
        local_fasterq_cache = os.environ.get(
            "AMALGKIT_LOCAL_FASTERQ_SCRATCH_DIR", ""
        ).strip()
        self.local_fasterq_scratch_dir = (
            Path(local_fasterq_cache).expanduser() if local_fasterq_cache else None
        )
        ...
        self.local_fasterq_scratch_min_gb = _nonnegative_float_setting(
            "AMALGKIT_LOCAL_FASTERQ_SCRATCH_MIN_GB", 2.0
        )
    ...
    def _validate_local_sra_archive(
        ...
        if validator is None:
            logger.warning(
                "vdb-validate is unavailable for local SRA preflight %s; "
                "continuing with fasterq-dump validation",
                srr_id,
            )
            return None
        ...
        try:
            ...
            result = _run_command_in_process_group(
                ...
                env=_ncbi_environment_for_data_root(_data_root()),
            )
            after = _sra_source_fingerprint(source)
        except subprocess.TimeoutExpired:
            logger.warning(
                "SRA preflight validation timed out for %s (>%s); "
                "continuing with fasterq-dump validation",
                srr_id,
            ...
            logger.warning(
                "SRA preflight validation could not inspect %s; "
                "continuing with fasterq-dump validation: %s",
                srr_id,
                exc,
            )
            return None
        ...
        if valid:
            ...
            logger.warning(
                "Skipping invalid local SRA archive for %s after vdb-validate "
                "exit %d: %s",
                srr_id,
                result.returncode,
    ...
    def _extract_sra_to_fastq_bounded(
        ...
        if semaphore is None:
            if (
                ...
                and self._validate_local_sra_archive(source, srr_id) is False
            ):
                return False
        ...
        if self.local_fasterq_scratch_dir is not None:
            if isinstance(source, Path):
                try:
                    ...
                    local_temp_reservation = max(
                        ...
                        math.ceil(
                            source_bytes
                            * (self.local_fasterq_scratch_multiplier + 1.0)
                        ),
                    )
            else:
                ...
                local_temp_reservation = int(
                    self.local_fasterq_remote_reserve_gb * (1024**3)
                )
            ...
            if local_temp_reservation and self._reserve_local_scratch(
                ...
                try:
                    ...
                    if local_temp_root.is_symlink() or (
                        local_temp_root.exists() and not local_temp_root.is_dir()
                    ):
                        ...
                        try:
                            if not _paths_share_filesystem(source, local_temp_root):
                                ...
                                copying_source = source_dir / (
                                    f".{source.name}.{os.getpid()}."
                                    f"{threading.get_ident()}.copying"
                                )
                    ...
                    logger.warning(
                        "Unable to prepare local fasterq scratch for %s; "
                        "using mounted-volume temporary storage: %s",
                        srr_id,
                        exc,
                    )
        ...
        *,
        ...
        while True:
            try:
                ...
                system_low = (
                    system_free_gb is not None
                    and system_free_gb < self.min_system_free_gb
                )
                if external_free_gb < self.min_external_free_gb or system_low:
                    if throttle_attempts % 6 == 0:  # Log every ~5 mins
                        system_text = (
                            f"; system {system_free_gb:.1f} GiB free"
                            if system_free_gb is not None
                            else ""
                        )
                        logger.warning(
                    ...
                    if throttle_attempts > 0:
                        logger.info(
                            "[Disk Throttle] Space available again (external %.1f GiB%s). "
                            "Resuming download for %s.",
                            external_free_gb,
                            (
                                f"; system {system_free_gb:.1f} GiB"
                                if system_free_gb is not None
                                else ""
                            ),
                            srr_id,
    ...
    def quant_sample(
        ...
        # Amalgkit 0.16.32; otherwise it rejects usable reads before Kallisto.
        ...
        *,
        ...
        with self._local_scratch_reservation_lock:
            ...
            available_after_reservation = (
                free_bytes - self._local_scratch_reserved_bytes - payload_bytes
            )
    ...
    def _prepare_local_quant_workspace(
        ...
        *,
        ...
        try:
            ...
            if staged_getfastq.is_symlink() or (
                staged_getfastq.exists() and not staged_getfastq.is_dir()
            ):
    ...
    def _reset_non_current_quantified_states(self, species_name: str) -> int:
        ...
        if validation_slots == 1:
            current_flags = [
                self.is_quantified(species_name, srr_id) for srr_id in sample_ids
            ]
        else:
            with concurrent.futures.ThreadPoolExecutor(
                max_workers=validation_slots
        ...
        for srr_id, is_current in zip(sample_ids, current_flags):
            if not is_current:
                self.db.set_state(
                    ...
                    srr_id,
                    "pending",
                    error="quantification lacks current 0.16.32 provenance",
                )
                reset += 1
        if len(sample_ids) > 1:
            logger.info(
    ...
    def run_tissue_normalization(self, metadata_path: Path) -> bool:
        ...
        try:
            ...
            if "tissue" not in df.columns:
                ...
                return True
            original_tissue = (
                df["tissue_original"].copy()
                if "tissue_original" in df.columns
                else df["tissue"].copy()
            )
    ...
    def discover_species_tasks(self, config_name: str, max_gb: float, threads: int) -> List[Dict[str, Any]]:
        ...
        if needs_prep:
            ...
            # Amalgkit 0.16.32 builds the quantification index through the
            ...
            # Taxonomy acquisition is owned by Amalgkit 0.16.32.  When the
        ...
        # Some valid Amalgkit 0.16.32 runs emit only ``metadata.tsv`` when
        ...
        if target_column:
            target_names = sorted(
                str(value).strip()
                for value in filtered[target_column].dropna().unique()
                if str(value).strip()
            )
        else:
        ...
        kmer_size = (
            _kallisto_index_kmer(str(reference_index))
            if reference_index
            else KALLISTO_DEFAULT_KMER_SIZE
        )
    ...
    def run_all(
        ...
        *,
        ...
        logger.info(
            "=== Phase 1 & 2: Incremental Task Discovery & Execution "
            "(requested workers=%d, active workers=%d) ===",
        ...
        with (
            ...
            concurrent.futures.ThreadPoolExecutor(
                max_workers=profile.fasterq_slots
            ...
            def record_terminal(task: Dict[str, Any], res: Dict[str, Any] | None, error: Exception | None = None) -> None:
            ...
            while (
                ...
                or fallback_futures
                or fallback_queue
                or ready_primary_tasks
                or not primary_exhausted
            ):
                submit_fallbacks()
