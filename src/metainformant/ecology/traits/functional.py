...
def community_weighted_mean(
    ...
    rel_abund = abundances / total if total > 0 else np.zeros_like(abundances, dtype=float)
...
def functional_richness(trait_values: np.ndarray) -> float:
    ...
    try:
        ...
        # scipy unavailable: no hull can be computed
        return 0.0
    ...
    try:
        ...
        # Degenerate (co-linear/co-planar) trait configurations
        return 0.0
