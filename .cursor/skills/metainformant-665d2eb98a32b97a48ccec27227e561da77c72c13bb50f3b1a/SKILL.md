---
name: metainformant-665d2eb98a32b97a48ccec27227e561da77c72c13bb50f3b1a
description: METAINFORMANT rules for directory projects/hymenoptera_amalgkit/doc/03_data_management. Use when editing, adding tests, or reviewing code under this path. Read the linked AGENTS.md first; use uv only, write outputs to output/, real implementations.
---

# METAINFORMANT — `projects/hymenoptera_amalgkit/doc/03_data_management`

Before editing files in this subtree:

- Read [`AGENTS.md`](../../../projects/hymenoptera_amalgkit/doc/03_data_management/AGENTS.md) for this folder (canonical technical context).
- Optional overview: [`README.md`](../../../projects/hymenoptera_amalgkit/doc/03_data_management/README.md).
- Global rules: [`CLAUDE.md`](../../../CLAUDE.md) at repo root (uv, `output/`, `.tmp/`, real implementations).
- Testing policy: [`docs/REAL_IMPLEMENTATION_POLICY.md`](../../../docs/REAL_IMPLEMENTATION_POLICY.md).
- Use `metainformant.core.io` for file I/O and `metainformant.core.utils.logging` for logs.

Keep changes scoped; match existing patterns in this directory.
